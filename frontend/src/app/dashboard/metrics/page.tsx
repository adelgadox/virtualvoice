    ...
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-5 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded-lg px-3 py-2 text-center">
          ...
          <p className="text-xs text-emerald-600 dark:text-emerald-500 mt-0.5">aprobación</p>
        </div>
        <div className="bg-blue-50 dark:bg-blue-950/30 rounded-lg px-3 py-2 text-center">
          ...
          <p className="text-xs text-blue-600 dark:text-blue-500 mt-0.5">editadas</p>
        </div>
      ...
      {m.total > 0 && (
        <div className="space-y-2">
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Aprobadas</p>
            <StatBar value={m.approved} max={m.total} color="bg-emerald-500" />
          </div>
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Editadas</p>
            <StatBar value={m.edited} max={m.total} color="bg-blue-500" />
          </div>
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Ignoradas</p>
            <StatBar value={m.ignored} max={m.total} color="bg-gray-400" />
          </div>
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Publicadas en Meta</p>
...
export default function MetricsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Métricas</h1>
        {!loading && metrics.length > 0 && (
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {totals.total} respuestas · {globalApprovalRate}% tasa de aprobación global
          </p>
        )}
      </div>
      ...
      {!loading && totals.total > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Aprobadas", value: totals.approved, color: "text-emerald-600 dark:text-emerald-400" },
            { label: "Editadas", value: totals.edited, color: "text-blue-600 dark:text-blue-400" },
            { label: "Ignoradas", value: totals.ignored, color: "text-gray-500 dark:text-gray-400" },
            { label: "Publicadas", value: totals.published, color: "text-violet-600 dark:text-violet-400" },
      ...
      {!loading && !error && metrics.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Sin datos todavía</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Las métricas aparecerán cuando haya respuestas procesadas</p>
        </div>
      )}
